...
type Guest = { id: number; name: string; group: string; status: "Confermato" | "In attesa" | "Non partecipa"; plus: number; invitationSent: boolean; needs: GuestNeeds };
      ...
      <main>
        ...
        <div className="content">
          ...
          {view === "guests" && <section className="page-section">
            ...
            <div className="panel guest-list">{filteredGuests.map((guest, index) => <div className="guest-row" key={guest.id}><div className={`guest-avatar g${index % 4}`}>{guest.name.charAt(0)}</div><div><b>{guest.name}{hasGuestNeeds(guest) && <span className="needs-badge" title={guestNeedsSummary(guest)}>Esigenze</span>}</b><small>{personalizedGroup(guest.group)} · {guest.plus} {guest.plus > 1 ? "persone" : "persona"}{witnesses.some(item => item.guestId === guest.id) && <span className="role-badge">Testimone</span>}</small></div><label className="invitation-switch"><input type="checkbox" checked={guest.invitationSent} onChange={e => setGuests(current => current.map(g => g.id === guest.id ? { ...g, invitationSent: e.target.checked } : g))} /><span aria-hidden="true"></span><em>{guest.invitationSent ? "Invito inviato" : "Da inviare"}</em></label><select value={guest.status} onChange={e => setGuests(guests.map(g => g.id === guest.id ? { ...g, status: e.target.value as Guest["status"] } : g))} aria-label={`Stato di ${guest.name}`}><option>Confermato</option><option>In attesa</option><option>Non partecipa</option></select><ItemMenu onEdit={() => openEdit("guest", guest.id)} onDelete={() => askDelete("guest", guest.id, guest.name)} /></div>)}</div>
          </section>}
